module.exports = function({ OperationType, VariableType, ConnectionState, AllowFlags, Callback, Utils, Room, Replay, RoomConfig, Plugin, Renderer, Impl }, n_A, canvas, images, renderApi){
  ...
  Ea.prototype = {
    ...
    C: function (a, b) {
      if (null != a.H) {
        var c = n_A.xm.L() ? b.kb[a.ea.$] : a.ea.wm, // "team_colors"
          d = null != a.Jd ? a.Jd : a.Xb,
          e = n_A.lm.L() && null != d; // "show_avatars"
        if (
          !Ea.Ln(this.kb, c) ||
          (!e && a.Jb != this.uh) ||
          (e && this.Jf != d)
  ...
  N.prototype = {
    uf: function () {
      var a = n_A.Tb.L(), // "view_mode"
        c = this;
      c.zg = n_A.Sl.L(); // "resolution_scale"
      0 == a
    ...
    Nq: function (a, b) {
      for (var c = n_A.Ak.L(), d = 0, e = a.I; d < e.length; ) { // "show_indicators"
        var f = e[d];
        ++d;
        var g = f.H;
        if (null != g) {
  ...
  this.render = function(extrapolatedRoomPhysicsObj){ // render logic here. called inside requestAnimationFrame callback
    if (renderApi?.paintGame){
      that.Eb.uf();
      null != extrapolatedRoomPhysicsObj.K && that.Eb.Kc(extrapolatedRoomPhysicsObj, that.roomObj.ya.uc);
    }
