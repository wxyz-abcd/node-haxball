module.exports = function(API, params){
  ...
  Renderer.call(this, { // Every renderer should have a unique name.
    ...
    version: "1.2",
    author: "basro & abc",
    description: `This is a customized and de-optimized renderer that can render players as images. Use +, - keys for zoom in-out. Disable followMode to zoom using mouse wheel.`
  });
  ...
  this.defineVariable({
    name: "redTeamImageUrl",
    description: "URL of the image that will be used for the red team.", 
  ...
  this.defineVariable({
    name: "rotatePlayers",
    description: "Rotate players by their current directions?", 
    type: VariableType.Boolean,
    value: true
  });
  ...
  this.defineVariable({
    name: "lookAtBall",
    description: "If activated, the direction of all players will always be towards the ball. Otherwise, they will look towards the direction of their current keys.", 
    type: VariableType.Boolean,
    value: false
  });
  ...
  var thisRenderer = this, { H: Point, p: Team, ka: TeamColors } = Impl.Core, roomLibrariesMap = null, imagesUpdated;
  ...
  PlayerDecorator.prototype = {
    ...
    update: function(playerObj, roomState){ // C
      if (playerObj.disc) {
        ...
        if (imagesUpdated || !/*PlayerDecorator.*/compareTeamColors(this.teamColors, teamColors) || (!showAvatar && (playerObj.avatarNumber!=this.avatarNumber)) || (showAvatar && (this.avatarText!=avatarText))){
          ...
          this.createInnerFillPattern(playerObj);
        }
      }
    ...
    createInnerFillPattern: function(playerObj){
      /*
      var colorArray = this.teamColors.inner;
      if (!colorArray || colorArray.length==0)
        return;
      */
      var img = imgs[playerObj.team.id-1];
      this.ctx.clearRect(0, 0, 64, 64);
      this.ctx.drawImage(img, 0, 0, img.width, img.height, 0, 0, 64, 64);
      /*
      // fill the colors
      this.ctx.save();
  ...
  HaxballRenderer.prototype = {
    ...
    render: function(roomState){ // Kc
      ...
      for (var i=0;i<playerObjects.length;i++){
        ...
        if (!playerDecorator){
          ...
          playerDecorator.playerObject = playerObject;
    ...
    drawDisc: function(disc, playerDecorator){ // Ll
      ...
      if (playerDecorator){
        ...
        this.ctx.scale(c, c);
        if (thisRenderer.rotatePlayers){
          var angle = 0;
          if (thisRenderer.lookAtBall){
            var ball = thisRenderer.room.state.gameState.physicsState.discs[0];
            if (ball)
              angle = Math.atan2(ball.pos.y - disc.pos.y, ball.pos.x - disc.pos.x);
          }
          else{
            var {dirX, dirY} = Utils.reverseKeyState(playerDecorator.playerObject.input);
  ...
  this.initialize = function(){
    ...
    img.src = thisRenderer.redTeamImageUrl;
    imgs.push(img);
    img = document.createElement("img");
    img.src = thisRenderer.blueTeamImageUrl;
    imgs.push(img);
  };
  ...
  */
  ...
  this.onVariableValueChange = function(addonObject, variableName, oldValue, newValue, customData){
    if (addonObject != thisRenderer || (variableName != "redTeamImageUrl" && variableName != "blueTeamImageUrl"))
      return;
    var img = document.createElement("img");
    img.src = thisRenderer[variableName];
    var t = (variableName == "redTeamImageUrl") ? 1 : 2;
    imgs[t-1] = img;
    imagesUpdated = true;
    var roomState = thisRenderer.room.state, playerObjects = roomState.players;
    for (var i=0;i<playerObjects.length;i++){
      var playerObject = playerObjects[i];
      if (!playerObject.disc || playerObject.team.id!=t)
        continue;
